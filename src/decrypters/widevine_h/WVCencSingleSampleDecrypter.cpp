...
widevine::Cdm::EncryptionScheme ToCdmEncryptionScheme(const CryptoMode cryptoMode)
{
  switch (cryptoMode)
  {
    case CryptoMode::NONE:
      return widevine::Cdm::EncryptionScheme::kClear;
    case CryptoMode::AES_CTR:
      return widevine::Cdm::EncryptionScheme::kAesCtr;
    case CryptoMode::AES_CBC:
      return widevine::Cdm::EncryptionScheme::kAesCbc;
    default:
      return widevine::Cdm::EncryptionScheme::kAesCtr;
  }
}
    ...
    m_host(host)
{
  ...
  std::string initData((const char*)m_pssh.data(), m_pssh.size());
  status = drm.GetCdmAdapter()->generateRequest(m_strSession, widevine::Cdm::InitDataType::kCenc,
                                                initData);
                                                   ...
                                                   IDecrypter::DecrypterCapabilites& caps)
{
  ...
  if ((caps.flags & IDecrypter::DecrypterCapabilites::SSD_SUPPORTS_DECODING) != 0)
  {
    ...
    m_fragmentPool[poolId].m_key =
        STRING::ToVecUint8(keyId.empty() ? m_keys.front().m_keyId : keyId);
...
void CWVCencSingleSampleDecrypter::CloseSessionId()
{
  if (!m_strSession.empty())
  {
    ...
    m_wvCdmAdapter.GetCdmAdapter()->close(m_strSession);
...
bool CWVCencSingleSampleDecrypter::SendSessionMessage()
{
  ...
  if (!blocks[3].empty() && !serverCertRequest)
  {
    if (blocks[3][0] == 'J' || (blocks[3].size() > 1 && blocks[3][0] == 'B' && blocks[3][1] == 'J'))
    {
      ...
      if (i < numTokens)
      {
        ...
        m_wvCdmAdapter.GetCdmAdapter()->update(m_strSession.data(), respData);
      }
      else
      {
    ...
    else if (blocks[3][0] == 'H' && blocks[3].size() >= 2)
    {
      ...
      if (payloadPos != std::string::npos)
      {
        ...
        if (blocks[3][1] == 'B')
        {
          std::string resp(response.c_str() + payloadPos, response.size() - payloadPos);
          m_wvCdmAdapter.GetCdmAdapter()->update(m_strSession.data(), resp);
        }
        else
        {
    ...
    else if (blocks[3][0] == 'B' && blocks[3].size() == 1)
    {
      ...
      m_wvCdmAdapter.GetCdmAdapter()->update(m_strSession.data(), decRespData);
    }
    else
    {
  ...
  {
    m_wvCdmAdapter.GetCdmAdapter()->update(m_strSession, response);
  }
                                                 ...
                                                 uint32_t status)
{
  ...
  res->status = static_cast<widevine::Cdm::KeyStatus>(status);
}
...
void CWVCencSingleSampleDecrypter::onMessage(widevine::Cdm::MessageType f_messageType,
                                             const std::string& f_message)
{
  switch (f_messageType)
  {
    ...
    default:
      LOG::LogF(LOGDEBUG, "unsupported message type %d", f_messageType);
      break;
  }
}
...
void CWVCencSingleSampleDecrypter::LogDecryptError(const widevine::Cdm::KeyStatus status,
                                                   const std::vector<uint8_t>& keyId)
{
  char buf[36];
  buf[32] = 0;
...
void CWVCencSingleSampleDecrypter::SetCdmSubsamples(
    std::vector<widevine::Cdm::Subsample>& subsamples, bool isCbc)
{
  widevine::Cdm::Subsample s;
  s.clear_bytes = 0;
  s.protected_bytes = m_decryptIn.GetDataSize();
  if (isCbc)
  {
    subsamples.resize(1);
    subsamples[0] = s;
...
void CWVCencSingleSampleDecrypter::SetInput(widevine::Cdm::InputBuffer& cdmInputBuffer,
                                            ...
                                            const std::vector<widevine::Cdm::Subsample>& subsamples)
{
  ...
  cdmInputBuffer.data_length = inputData.GetDataSize();
  cdmInputBuffer.iv = iv;
  cdmInputBuffer.iv_length = 16; //Always 16, see AP4_CencSingleSampleDecrypter declaration.
  ...
  cdmInputBuffer.subsamples_length = subsampleCount;
                                                           ...
                                                           const AP4_UI32* bytesOfEncryptedData)
{
  ...
  widevine::Cdm::KeyStatus ret{widevine::Cdm::Status::kSuccess};
  ...
  for (size_t i = 0; i < iterations; ++i)
  {
    ...
    widevine::Cdm::InputBuffer cdmIn;
    ...
    output.data = const_cast<uint8_t*>(m_decryptOut.GetData());
    output.data_length = m_decryptOut.GetDataSize();

    widevine::Cdm::Sample samples[1];
    samples[0].input = cdmIn;
    samples[0].output = output;

    widevine::Cdm::DecryptionBatch batch;
    batch.key_id = fragInfo.m_key.data();
    batch.key_id_length = fragInfo.m_key.size();
    batch.pattern.encrypted_blocks =
        fragInfo.m_cryptoInfo.m_cryptBlocks; //pattern.encrypted_blocks;
    batch.pattern.clear_blocks = fragInfo.m_cryptoInfo.m_skipBlocks; //pattern.clear_blocks;
    batch.samples = samples;
    batch.samples_length = 1;
    batch.encryption_scheme = ToCdmEncryptionScheme(fragInfo.m_cryptoInfo.m_mode);

    CheckLicenseRenewal();
    widevine::Cdm::Status decrypt_status =
        m_wvCdmAdapter.GetCdmAdapter()->decrypt(m_strSession, batch);

    // ret = m_wvCdmAdapter.GetCdmAdapter()->decrypt(cdmIn, &cdmOut);

    if (ret == widevine::Cdm::Status::kSuccess)
    {
      size_t cipherPos = 0;
      if (useCbcDecrypt)
      {
  ...
  return (ret == widevine::Cdm::Status::kSuccess) ? AP4_SUCCESS : AP4_ERROR_INVALID_PARAMETERS;
}
...
bool CWVCencSingleSampleDecrypter::Decrypt(const DEMUX_PACKET* sampleIn)
{
  ...
  DEMUX_CRYPTO_INFO* cInf = sampleIn->cryptoInfo;
  widevine::Cdm::Pattern patern;
  patern.encrypted_blocks = cInf->cryptBlocks;
  patern.clear_blocks = cInf->skipBlocks;
  uint32_t f_pcbOpaqueClearContent;
  uint8_t* f_ppbOpaqueClearContent;
  return Decrypt(ToCdmEncryptionScheme((CryptoMode)cInf->mode), patern, cInf->iv, 16,
                 sampleIn->pData, sampleIn->iSize, &f_pcbOpaqueClearContent,
                 &f_ppbOpaqueClearContent, 16, cInf->kid, false);
}

bool CWVCencSingleSampleDecrypter::Decrypt(const widevine::Cdm::EncryptionScheme encryptionScheme,
                                           ...
                                           bool initWithLast15)
{
  ...
  *f_pcbOpaqueClearContent = 0;
  ...
  if (widevine::Cdm::kSuccess == m_wvCdmAdapter.GetCdmAdapter()->getKeyStatuses(m_strSession, &map))
  {
    ...
    if (widevine::Cdm::kUsable == it->second)
    {
      ...
      widevine::Cdm::Subsample subsamples[1];
      subsamples[0].protected_bytes = f_cbData;
      ...
      input.subsamples_length = 1;

      widevine::Cdm::Sample samples[1];
      samples[0].input = input;
      samples[0].output = output;

      widevine::Cdm::DecryptionBatch batch;
      batch.key_id = reinterpret_cast<const uint8_t*>((it->first).c_str());
      batch.key_id_length = (it->first).size();
      batch.pattern.encrypted_blocks = pattern.encrypted_blocks;
      batch.pattern.clear_blocks = pattern.clear_blocks;
      batch.samples = samples;
      batch.samples_length = 1;
      batch.encryption_scheme = encryptionScheme;

      widevine::Cdm::Status decrypt_status =
          m_wvCdmAdapter.GetCdmAdapter()->decrypt(m_strSession, batch);
      if (widevine::Cdm::kSuccess == decrypt_status)
      {
      ...
      {
        LOG::Log(LOGERROR, "Decryption failed: %d\n", decrypt_status);
      }
    }
  }
  return status;
...
void CWVCencSingleSampleDecrypter::AddKeyId(std::string_view keyId)
{
  WVSKEY key;
  key.m_keyId = keyId;
  key.status = widevine::Cdm::KeyStatus::kUsable;
