...
void CURL::Parse(std::string strURL1)
{
  ...
  std::string strURL = strURL1; //CUtil::ValidatePath(std::move(strURL1));
  ...
  if (IsProtocol("stack") || IsProtocol("virtualpath") || IsProtocol("multipath") ||
      IsProtocol("special") || IsProtocol("resource"))
  {
...
std::string CURL::Get() const
{
  ...
  unsigned int sizeneed = m_strProtocol.length() + m_strDomain.length() + m_strUserName.length() +
                          m_strPassword.length() + m_strHostName.length() + m_strFileName.length() +
...
std::string CURL::GetWithoutOptions() const
{
  ...
  if (m_strFileName.size() && (m_strFileName[0] == '/' || m_strFileName[0] == '\\') &&
...
{
  return false; //::GetNetwork().IsLocalHost(m_strHostName);
}
...
std::string CURL::Encode(std::string_view strURLData)
{
  ...
  for (auto kar : strURLData)
  {
    ...
    if (StringUtils::isasciialphanum(kar) || kar == '-' || kar == '.' || kar == '_' || kar == '!' ||
        kar == '(' || kar == ')')
