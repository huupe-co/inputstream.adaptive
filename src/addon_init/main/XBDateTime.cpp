...
bool CDateTime::SetFromW3CDateTime(const std::string& dateTime, bool ignoreTimezone /* = false */)
{
  ...
  if (!ignoreTimezone && !zone.empty())
  {
    ...
    else
    {
      ...
      CDateTimeSpan zoneSpan;
      zoneSpan.SetFromTimeString(zone.substr(1));
...
//       size_t pos = i + 1;
//       while (((pos = strFormat.find(c, pos + 1)) != std::string::npos && pos < strFormat.size()) &&
//              strFormat[pos + 1] == '\'')
//       {
//       }
...
int CDateTime::MonthStringToMonthNum(const std::string& month)
{
  ...
  const char* abr_months[] = {"jan", "feb", "mar", "apr", "may", "jun",
                              "jul", "aug", "sep", "oct", "nov", "dec"};

  int i = 0;
  for (; i < 12 && !StringUtils::EqualsNoCase(month, months[i]) &&
         !StringUtils::EqualsNoCase(month, abr_months[i]);
       i++)
    ;
  i++;
